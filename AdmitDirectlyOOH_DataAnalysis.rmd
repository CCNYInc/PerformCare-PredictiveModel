...
OOH_LOS<- tbl_long_cols_2(con, "RSPM_OOH_LOS")%>%
    collect()
...
OOH_SiteGr <- AdmitDirectlyOOH %>%
     select(memberid,SiteGroup,SiteType)%>%
...
##This Informations are from RSPM_LOS table which contain all the  LOS for the members

```{r}
summary(OOH_LOS$LOS)
# mean(df_los$lengthofstay)
boxplot(LOS , data=OOH_LOS)
```
